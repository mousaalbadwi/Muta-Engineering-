...
namespace MutaEngineering.Data
{
    public static class DbSeeder
    {
        public static async Task SeedAsync(IServiceProvider services)
        {
            ...
            // نفّذ المايغريشن فقط إذا في pending (تجنّب بطء الإقلاع)
            if ((await db.Database.GetPendingMigrationsAsync()).Any())
                await db.Database.MigrateAsync();

            // لو في بيانات مسبقاً، اطلع فوراً
            ...
            var D = deps.ToDictionary(x => x.Code!, x => x.Id);
     ...
     new FacultyMember
     {
         ...
         Office = "C-115",
         PhotoPath = "~/img/faculty/avatar.png",
         DepartmentId = D["ELEC"]
     },
     new FacultyMember
     {
     ...
     new FacultyMember
     {
         ...
         DepartmentId = D["COMP"]
            ...
            // ===== أرشيف امتحانات (تأكد من وجود الملفات تحت wwwroot/docs/archive) =====
